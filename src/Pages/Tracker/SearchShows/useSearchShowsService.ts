...
const useSearchShowsService = () => {
    ...
    const getTvShows = (queryString: string) => axios.get(searchTvShows, {
        params: {
            query: queryString,
            language: 'zh-CN',
        },
    ...
    const postShow = (item: ShowDetailType) => axios.post(addShowUrl, {
        records: [
            {
                fields: {
                    ID: item.id,
    ...
    return {
        addShow, search, searchStreamProviders: searchSteamProviders, result, loading, addingShow, addedShows
    }
}
